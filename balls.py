...
class Ball():
    ...
    def __init__(self, r, color):
        ...
        self.r = r
        self.color = color
        self.obj = canv.create_oval(self.x - r, self.y - r, \
                                    self.x + r, self.y + r, \
                                        fill = color, width = 0)
    ...
    def collision(self, other):
            ...
            elif self.x - other.x > 0 and self.y - other.y > 0 \
                 or self.x - other.x < 0 and self.y - other.y < 0:
            ...
            v1 = (vx1 ** 2 + vy1 ** 2) ** 0.5
            v2 = (vx2 ** 2 + vy2 ** 2) ** 0.5
            if vy1 !=0: a1 = math.acos(vx1 / v1) * vy1/abs(vy1)
            else: a1 = math.acos(vx1 / v1)
            if vy2 != 0: a2 = math.acos(vx2 / v2) * vy2/abs(vy2)
            else: a2 = math.acos(vx2 / v2)
...
def click(event):
    ...
    for i in range(len(balls)):
        if ((balls[i].x - event.x) ** 2 + (balls[i].y - event.y) ** 2) ** (1 / 2) <= balls[i].r:
            k = i
    if k >= 0:
